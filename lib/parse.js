...
function addValueByNodeType (node, mod) {
    if (node.type === 'ObjectExpression') {
        ...
        addCallExpression(node, mod);

    } else if (node.type === 'FunctionExpression') {
        comments.addClosest(node, mod);
        addMethodValue(mod, node, node.parent.parent.leadingComments);
        ...
        if (b.type === 'FunctionDeclaration') {
            ...
            comments.addClosest(b, mod);
            addMethodValue(mod, b, b.parent.parent.leadingComments);
            addAdditionalProperties(b.parent.parent.parent.body, node.name, b.parent.parent, mod);
        ...
        addValueByNodeType(rightmost.right, mod);
...
function addCallExpression (node, mod) {
        ...
        utils.each(utils.getPropertyIfPresent(node, 'callee.body.body'), function (b) {
            if (b.type === 'ReturnStatement') {
                if (b.argument.type === 'Identifier') {
                    comments.addClosest(b, mod);
                    addAdditionalProperties(b.parent.body, b.argument.name, b, mod);

                } else {
                    addValueByNodeType(b.argument, mod);
                }
    ...
    utils.each(properties, function (prop) {
        if (prop.key && prop.value) {
            ...
            if (prop.value.type === 'Identifier') {
                ...
                if (b.type === 'FunctionDeclaration') {
                    addMethodValue(parsedProp, b, b.leadingComments);

                } else if (b.type === 'FunctionExpression') {
                    addMethodValue(parsedProp, b, b.parent.parent.leadingComments);

                } else if (b.type === 'CallExpression') {
                    addCallExpression(b, parsedProp);
                ...
                addMethodValue(parsedProp, prop.value, prop.value.leadingComments || prop.leadingComments);
...
function addMethodValue (parsedObj, node, leadingComments) {
    ...
    parsedObj.signature += getMehodSignature(node.params);
    parsedObj.labels = getLabels(parsedComments.labels);
    parsedObj.params = getMethodParams(node.params, parsedComments);
    parsedObj.returns = getMethodReturns(parsedComments.returns, node);
}
...
function getMethodReturns (returns, node) {
    if (returns) {
        var rets = [];
    ...
    var unknownReturn;
    var fnBody = utils.getPropertyIfPresent(node, 'body.body');
    utils.each(fnBody, function (b) {
        if (b.type === 'ReturnStatement') {
            unknownReturn = [{
                _classname: 'readmejs-return',
                description: 'Unspecified ' + b.argument.type
            }];
            return false;
        }
    });
...
function rightSide (node, modDefaults) {
    var mod = modDefaults || {};

    addValueByNodeType(node.right, mod);
...
function parseRequired (ext, mod) {
    ...
    traverse(ast, { pre: function (node, parent) {
        return parseNodeAsExport(mod, ext.path, node, parent, mods);
    } });
...
function parseNodeAsExport (options, filepath, node, parent, collection) {
    if (node.type === 'AssignmentExpression') {
        var mod = {
        ...
        if (isExport) {
            Object.assign(mod, rightSide(node, mod));
...
function parseEntry (options) {
    ...
    traverse(ast, { pre: function (node, parent) {
        return parseNodeAsExport(options, filepath, node, parent, app.api);
    } });
...
function parseLib (options) {
    var libPath = options.src;

    var app = newBlankApp(options);

    if (!utils.pathExists(libPath)) {
        console.log('\nERROR: options.src must be a directory');
        return;
    }
    ...
    if (listFiles.length) {
        listFiles.forEach(function (filepath) {
            ...
            traverse(ast, { pre: function (node, parent) {
                return parseNodeAsExport(options, filepath, node, parent, app.lib);
            } });
