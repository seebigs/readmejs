...
var parseComments = require('./parse_comments.js');
...
// function parseValidCommentsOld (comments) {
...
function addPropertyValue (parsedProp, parsedComments, val) {
    ...
    if (!parsedProp.comments || !parsedProp.comments.length) {
    ...
    utils.each(properties, function (prop) {
        if (prop.key && prop.value) {
            ...
            if (prop.value.type === 'Identifier') {
                ...
                addPropertyValue(parsedProp, {}, prop.value);
            }
...
function addMethodValue (parsedValue, paramsNode, parsedComments) {
    ...
    if (!parsedValue.comments && parsedComments.description) {
        parsedValue.comments = parsedComments.description;
    }
    parsedValue.params = getMethodParams(paramsNode, parsedComments);
    parsedValue.signature += getMehodSignature(parsedValue.params);
    parsedValue.returns = parsedComments.returns;
}
...
function addValueByNodeType (node, ast, parsedValue) {
    ...
    if (node.type === 'ObjectExpression') {
        addProperties(parsedValue, node.properties, parsedValue.comments || getComments(node.parent), ast);
        ...
        if (b.type === 'FunctionDeclaration') {
            ...
            var comment = getComments(b.parent.parent);
            if (comment.description) {
                parsedValue.comments = comment.description;
            }
            if (comment.name) {
                parsedValue.name = comment.name;
            }
