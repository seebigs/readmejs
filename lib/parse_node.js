...
function addValueByNodeType (node, ast, mod) {
    ...
    if (node.type === 'ObjectExpression') {
        ...
        comments.addClosest(node, mod);
        utils.debug('FunctionExpression');
        addMethodValue(mod, node.params, {});
        ...
        utils.debug('AssignmentExpression');
        // var rightmost = getRightmostAssignment(node);
        // return rightSide(rightmost, ast, mod);
        ...
        if (b.type === 'FunctionDeclaration') {
            ...
            //     mod.comments = comment.description;
            // }
            // if (comment.name) {
            //     mod.name = comment.name;
            // }
...
function addMethodValue (parsedObj, paramsNode, leadingComments) {
    ...
    parsedObj.returns = parsedComments.returns;
}
